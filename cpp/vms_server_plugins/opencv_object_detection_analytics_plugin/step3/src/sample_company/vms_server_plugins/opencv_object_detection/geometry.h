...
inline cv::Rect
nxRectToCvRect(nx::sdk::analytics::Rect rect, int width, int height)
{
  ...
  return cv::Rect(
    ...
    cv::Rect(
           0, 0, width, height
    ); //< Ensure that the result is inside the frame rect.
}

inline nx::sdk::analytics::Rect
cvRectToNxRect(cv::Rect rect, int width, int height)
{
  if (!NX_KIT_ASSERT(width > 0) || !NX_KIT_ASSERT(height > 0))
    return {};
