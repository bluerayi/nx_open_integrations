 ...
 * - pushMetadataPacket() is called by the plugin, while pullMetadataPackets()
 * is called by Server.
 * - pushMetadataPacket() expects one metadata packet, while pullMetadataPacket
 * expects the std::vector of them.
 *
 * There are no strict rules for deciding which method is "better". A rule of
 * thumb is to use pushMetadataPacket() when you generate one metadata packet
 * and do not want to store it in the class field, and use pullMetadataPackets
 * otherwise.
 */
bool
DeviceAgent::pullMetadataPackets(
...
Ptr<IMetadataPacket>
DeviceAgent::generateEventMetadataPacket()
{
  ...
  eventMetadata->setDescription(
    "New track #" + std::to_string(m_trackIndex) + " started"
  );
...
Ptr<IMetadataPacket>
DeviceAgent::generateObjectMetadataPacket()
{
  ...
  // Calculate bounding box coordinates each frame so that it moves from the top
  // left corner to the bottom right corner during kTrackFrameCount frames.
