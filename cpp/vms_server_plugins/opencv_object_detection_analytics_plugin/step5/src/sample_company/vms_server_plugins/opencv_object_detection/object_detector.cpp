...
/**
 * Load the model if it is not loaded, do nothing otherwise. In case of errors
 * terminate the plugin and throw a specialized exception.
 */
void
ObjectDetector::ensureInitialized()
{
  ...
  try {
    ...
    terminate();
    throw ObjectDetectorInitializationError(
      "Loading model: " + cvExceptionToStdString(e)
    );
  } catch (const std::exception & e) {
    terminate();
    throw ObjectDetectorInitializationError(
      "Loading model: Error: "s + e.what()
    );
  }
}

bool
ObjectDetector::isTerminated() const
{
  return m_terminated;
}

void
ObjectDetector::terminate()
{
  m_terminated = true;
}

DetectionList
ObjectDetector::run(const Frame & frame)
{
  if (isTerminated())
    throw ObjectDetectorIsTerminatedError(
      "Detection error: object detector is terminated."
    );
...
ObjectDetector::loadModel()
{
  ...
  static const auto modelBin =
    m_modelPath / std::filesystem::path("MobileNetSSD.caffemodel");
  static const auto modelTxt =
    m_modelPath / std::filesystem::path("MobileNetSSD.prototxt");
...
convertRawDetectionToDetection(const Mat & rawDetections, int detectionIndex)
{
  ...
  const float confidence =
    rawDetections.at<float>(i, (int)OutputIndex::confidence);
  const auto classIndex =
    (int)(rawDetections.at<float>(i, (int)OutputIndex::classIndex));
  const std::string classLabel = kClasses[(size_t)classIndex];
  const bool confidentDetection = confidence >= confidenceThreshold;
  const bool oneOfRequiredClasses =
  ...
  if (confidentDetection && oneOfRequiredClasses) {
    const float xBottomLeft =
      rawDetections.at<float>(i, (int)OutputIndex::xBottomLeft);
    const float yBottomLeft =
      rawDetections.at<float>(i, (int)OutputIndex::yBottomLeft);
    const float xTopRight =
      rawDetections.at<float>(i, (int)OutputIndex::xTopRight);
    const float yTopRight =
      rawDetections.at<float>(i, (int)OutputIndex::yTopRight);
    ...
    return std::make_shared<Detection>(Detection {
      ...
      classLabel,
      confidence,
      /*trackId*/ nx::sdk::Uuid() //< Will be filled with real value in
                                  //ObjectTracker.
    });
  }
  return nullptr;
...
DetectionList
ObjectDetector::runImpl(const Frame & frame)
{
  if (isTerminated()) {
    throw ObjectDetectorIsTerminatedError(
  ...
  const Mat netInputBlob =
    blobFromImage(image, scaleFactor, netInputImageSize, mean);
  ...
  for (int i = 0; i < detections.rows; ++i) {
    const std::shared_ptr<Detection> detection =
      convertRawDetectionToDetection(detections, i);
    if (detection)
