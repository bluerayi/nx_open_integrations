...
/**
 * @param deviceInfo Various information about the related device, such as its
 * id, vendor, model, etc.
 */
DeviceAgent::DeviceAgent(
  ...
  : // Call the DeviceAgent helper class constructor telling it to verbosely
    // report to stderr.
...
/**
 *  @return JSON with the particular structure. Note that it is possible to fill
 * in the values that are not known at compile time, but should not depend on
 * the DeviceAgent settings.
 */
std::string
DeviceAgent::manifestString() const
{
  // Tell the Server that the plugin can generate the events and objects of
  // certain types. Id values are strings and should be unique. Format of ids:
 ...
 */
bool
DeviceAgent::pushUncompressedVideoFrame(
...
void
DeviceAgent::doSetNeededMetadataTypes(
...
DeviceAgent::MetadataPacketList
DeviceAgent::eventsToEventMetadataPacketList(
...
)
{
  ...
  for (const std::shared_ptr<Event> & event : events) {
    ...
    if (
      event->eventType == EventType::detection_started ||
      ...
      const std::string suffix =
        (event->eventType == EventType::detection_started) ? kStartedSuffix
                                                           : kFinishedSuffix;
      const std::string caption =
        kClassesToDetectPluralCapitalized.at(event->classLabel) + " detection" +
        suffix;
      const std::string description = caption;

      eventMetadata->setCaption(caption);
      eventMetadata->setDescription(description);
      eventMetadata->setIsActive(
        event->eventType == EventType::detection_started
      );
      ...
      std::string description =
        event->classLabel + kDetectionEventDescriptionSuffix;
      ...
      objectDetectedEventMetadataPacket->addItem(eventMetadata);
    }
  }
...
Ptr<ObjectMetadataPacket>
DeviceAgent::detectionsToObjectMetadataPacket(
...
)
{
  ...
  for (const std::shared_ptr<Detection> & detection : detections) {
    ...
    // There is no "else", because only the detections with those types are
    // generated.
...
void
DeviceAgent::reinitializeObjectTrackerOnFrameSizeChanges(const Frame & frame)
{
  const bool frameSizeUnset =
    m_previousFrameWidth == 0 && m_previousFrameHeight == 0;
  if (frameSizeUnset) {
...
DeviceAgent::MetadataPacketList
DeviceAgent::processFrame(Ptr<const IUncompressedVideoFrame> videoFrame)
{
  ...
  try {
    ...
    ObjectTracker::Result objectTrackerResult =
      m_objectTracker->run(frame, detections);
    const auto & objectMetadataPacket = detectionsToObjectMetadataPacket(
      objectTrackerResult.detections, frame.timestampUs
    );
