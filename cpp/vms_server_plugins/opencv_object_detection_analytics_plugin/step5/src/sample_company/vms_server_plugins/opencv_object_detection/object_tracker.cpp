...
/**
 * This function implementation is based on the sample from opencv_contrib
 * repository:
...
createTrackerByMatchingWithFastDescriptor()
{
  ...
  // Keep forgotten tracks for cleaning up our tracks and dropping
  // cv::detail::tracking::tbm tracks manually.
...
public:
  explicit Track(int maxDetectionCount)
  ...
  {
    return m_detections.begin()->second->classLabel;
  };
...
ObjectTracker::run(const Frame & frame, const DetectionList & detections)
{
  try {
...
EventList
ObjectTracker::generateDetectionFinishedEvents(int64_t timestampUs)
{
  EventList result;
  for (const std::string & classLabel : kClassesToDetect) {
...
void
ObjectTracker::copyDetectionsHistoryToTrack(
  const Frame & frame,
  int64_t cvTrackId,
  Track * track,
...
{
  const cv::detail::tracking::tbm::Track & cvTrack =
    m_tracker->tracks().at((size_t)cvTrackId);
  for (const TrackedObject & trackedDetection : cvTrack.objects) {
    if ((int64_t)trackedDetection.timestamp == frame.timestampUs)
      break;

    std::shared_ptr<const DetectionInternal> detection =
      convertTrackedObjectToDetection(
        /*frame*/ frame,
...
std::shared_ptr<ObjectTracker::Track>
ObjectTracker::getOrCreateTrack(const Uuid & trackId)
{
  std::shared_ptr<Track> result;
  if (m_tracks.find(trackId) == m_tracks.end()) {
    result =
      std::make_shared<Track>(m_tracker->params().max_num_objects_in_track);
...
EventList
ObjectTracker::processDetection(
  const Frame & frame,
...
)
{
  ...
  const std::string & classLabel = detection->detection->classLabel;
  if (
    (trackStatus == Track::Status::started ||
     trackStatus == Track::Status::active) &&
...
EventList
ObjectTracker::generateEvents(
  const Frame & frame,
  const DetectionInternalList & detectionsInternal
)
{
  EventList result;
  for (const std::shared_ptr<DetectionInternal> & detection :
       detectionsInternal) {
  ...
  const EventList detectionFinishedEvents =
    generateDetectionFinishedEvents(frame.timestampUs);
  result.insert(
    result.end(),
...
ObjectTracker::Result
ObjectTracker::runImpl(const Frame & frame, const DetectionList & detections)
{
  // Unfortunately the OpenCV tbm module does not support preserving classLabel
  // during tracking. See issue:
  // https://github.com/opencv/opencv_contrib/issues/2298 Therefore, we save
  // information about classLabels in the map from unique id of the detection
  ...
  m_tracker->process(
    frame.cvMat, detectionsToTrack, (uint64_t)frame.timestampUs
  );
  const TrackedObjects trackedDetections = m_tracker->trackedDetections();

  DetectionInternalList detectionsInternal = convertTrackedObjectsToDetections(
    /*frame*/ frame,
