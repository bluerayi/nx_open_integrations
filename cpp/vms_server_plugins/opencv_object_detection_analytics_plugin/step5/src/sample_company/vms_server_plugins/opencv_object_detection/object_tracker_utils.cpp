...
void
IdMapper::removeAllExcept(const std::set<Uuid> & idsToKeep)
{
...
/**
 * Convert detections from the plugin format to the format of
 * opencv::detail::tracking::tbm, preserving classLabels.
 */
TrackedObjects
convertDetectionsToTrackedObjects(
  const Frame & frame,
...
)
{
  ...
  for (const std::shared_ptr<Detection> & detection : detections) {
    const cv::Rect cvRect =
      nxRectToCvRect(detection->boundingBox, frame.width, frame.height);

    inOutClassLabels->insert(
      std::make_pair(
        CompositeDetectionId {frame.index, cvRect}, detection->classLabel
      )
    );
    ...
    result.push_back(TrackedObject(
      ...
      (int)frame.index,
      /*object_id*/ -1
    )); //< Placeholder, to be filled in ObjectTracker::process().
  }
 ...
 */
std::shared_ptr<DetectionInternal>
...
)
{
  auto detection = std::make_shared<Detection>(
    Detection {/*boundingBox*/ cvRectToNxRect(
                 trackedDetection.rect, frame.width, frame.height
               ),
               classLabel,
...
/**
 * Convert detections from opencv::detail::tracking::tbm format to the plugin
 * format, restoring classLabels.
 */
DetectionInternalList
...
)
{
  ...
  for (const cv::detail::tracking::tbm::TrackedObject & trackedDetection :
       trackedDetections) {
    const std::string classLabel =
      classLabels.at({frame.index, trackedDetection.rect});
    result.push_back(convertTrackedObjectToDetection(
      frame, trackedDetection, classLabel, idMapper
    ));
  }
...
DetectionList
extractDetectionList(const DetectionInternalList & detectionsInternal)
{
